...
using Lfm.Interface.Cli.CommandBuilders;
    ...
    static IHostBuilder CreateHostBuilder(string[] args) =>
        Host.CreateDefaultBuilder(args)
            .ConfigureServices((context, services) =>
            {
                ...
                services.AddTransient<Lfm.Integration.Spotify.IPlaylistStreamer>(serviceProvider =>
                {
                    var configManager = serviceProvider.GetRequiredService<IConfigurationManager>();
                    var config = configManager.LoadAsync().GetAwaiter().GetResult();
                    return new Lfm.Integration.Spotify.SpotifyStreamer(config.Spotify, configManager);
                });

                // Sonos services
                services.AddTransient<Lfm.Integration.Sonos.ISonosStreamer>(serviceProvider =>
                {
                    var configManager = serviceProvider.GetRequiredService<IConfigurationManager>();
                    var config = configManager.LoadAsync().GetAwaiter().GetResult();
                    var logger = serviceProvider.GetRequiredService<ILogger<Lfm.Integration.Sonos.SonosStreamer>>();
                    return new Lfm.Integration.Sonos.SonosStreamer(config.Sonos, logger);
                });
                ...
                services.AddTransient<ILastFmService>(serviceProvider =>
                {
                    ...
                    var spotifyStreamer = serviceProvider.GetRequiredService<Lfm.Integration.Spotify.IPlaylistStreamer>();
